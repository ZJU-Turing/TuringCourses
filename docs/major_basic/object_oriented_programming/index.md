...
=== "陈翔"
    ...
    没有任何形式的考勤。平时作业包括 7 次 assignment 和 7 次作业，每周交叉布置。Assignment 类似 lab，不过是主观题的形式由助教批改，无报告，要求比较宽松，自由发挥的空间比较大。作业一般是一或两道填空题或程序填空题，都比较简单。大作业是实现一个序列化和反序列化的库，要求支持二进制和 XML 两种格式。从历两年来看助教在批改作业的给分上相当慷慨，平时分基本上都是满分（22 春夏大作业在 PTA 上标为 60 分，满分比例是 88/142，55 分及以上比例是 109/142）。
