...
- 对于外部资源，请插入链接，不要将文件传入本 repo
- 请不要上传有版权的文件，例如课件等
- 对于自己的笔记、复习提纲等材料：
    - 如果有自己的网站，推荐放在自己的网站并在此插入链接
    - 如果不方便链接，可以上传到对应课程文件夹中（如果是 markdown 文档同时推荐更新 nav）
...
#### 直接提交
对于在 Organization 中的同学，如果觉得 PR 过程有些复杂，也可以直接修改、提交到本仓库中（可以在线修改，也可以 clone 到本地修改然后 commit、push）。如果在提交中存在问题，我们后续会及时进行修改。
