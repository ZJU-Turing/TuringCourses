...
=== "陈越"
    ...
    - bonus：4分，这四分 Bonus 用于补平时分，平时分补满60分为止。
...
=== "何钦铭"
    ...
    平时分四个部分加起来和 50 取 min（不溢出），期中 project 分数不论 normal 还是 hard 都是 100 分满。但是选了 hard 的可以在平时分上额外多加 <project 分数（100 满）> * 0.05 分。（注意，hard 和 bouns 的分数均无法溢出到总评）
