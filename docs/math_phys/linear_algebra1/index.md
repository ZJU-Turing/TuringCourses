...
=== "冯涛"
    ...
    冯涛老师讲课侧重基础算法，会跳过过于技术性的部分（例如惯性定理的证明）和抽象化的部分（例如线性空间的八条公理）。另外因为面向工科生，上课难度较低，因此期末考前可以提前一段时间check一下历年卷的风格和难度。平时工作量比较小，可能要补充部分课后题训练技巧（涉及到的知识点是覆盖的，老师也鼓励大家做课后题）。根据自身体验给分是好的。老师的板书比较完善，如果复习板书不必担心遗漏。
